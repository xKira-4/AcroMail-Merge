...
if (app["viewerVersion"] < 10) {
  ...
  // OPTIMIZED: Only scan new instances, reuse existing field data
  function getXFAFieldMapDirect(doc, cachedMap, cachedInfo, currentInstanceCount) {
    var fieldMap = cachedMap || {};
    var sectionInfo = cachedInfo || {};
    
    try {
      var xfaForm = doc.xfa.form;
      
      // Only scan Page1 if not cached
      if (!cachedMap) {
        try {
      ...
      if (page2Node) {
        ...
        var startInstance = (currentInstanceCount !== undefined) ? currentInstanceCount : 0;
        
        // Only scan new instances
        for (var i = startInstance; i < instanceCount; i++) {
          try {
  ...
  function mailMergeXFAFiles(doc) {
    ...
    // OPTIMIZATION: Cache field map and only update when instances change
    var cachedFieldMap = null;
    var cachedSectionInfo = null;
    var cachedInstanceCount = 0;
    ...
    for (var groupIndex = 0; groupIndex < recipientGroups.length; groupIndex++) {
      ...
      var instancesNeeded = 0;
      
      // Build initial field map only once
      if (!cachedFieldMap) {
        var mapResult = getXFAFieldMapDirect(doc, null, null, 0);
        cachedFieldMap = mapResult.fieldMap;
        cachedSectionInfo = mapResult.sectionInfo;
        cachedInstanceCount = currentInstances;
      }
      
      instancesNeeded = calculateInstancesNeeded(rows.length, cachedSectionInfo);
      ...
      if (instancesNeeded > currentInstances) {
        ...
        // Only scan the NEW instances
        var mapResult = getXFAFieldMapDirect(doc, cachedFieldMap, cachedSectionInfo, cachedInstanceCount);
        cachedFieldMap = mapResult.fieldMap;
        cachedSectionInfo = mapResult.sectionInfo;
        cachedInstanceCount = instancesNeeded;
        
      } else if (instancesNeeded < currentInstances) {
        removeExcessPages(doc, instancesNeeded);
        
        // Remove fields from removed instances
        for (var fieldName in cachedFieldMap) {
          var fieldInstances = cachedFieldMap[fieldName];
          var newInstances = [];
          for (var fi = 0; fi < fieldInstances.length; fi++) {
            if (fieldInstances[fi].section === "Page1" || fieldInstances[fi].section < instancesNeeded) {
              newInstances.push(fieldInstances[fi]);
            }
          }
          cachedFieldMap[fieldName] = newInstances;
        }
        
        // Remove section info for removed instances
        for (var s in cachedSectionInfo) {
          if (s !== "Page1" && parseInt(s) >= instancesNeeded) {
            delete cachedSectionInfo[s];
          }
        }
      ...
      var xfaFieldMap = cachedFieldMap;
      var sectionInfo = cachedSectionInfo;
